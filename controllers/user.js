   ...
   connectDB.query(data, (err, result) => {
      if (err) throw err; // show if any error have
         ...
         if (result.length) {
            ...
            res.render('user/loginAccount', { user: "", msg: [], err: ["Username/Password salah"] });
         }
...
exports.postStatus = (req, res, next) => {
   ...
   var checkOutDate = req.body.date2
   //console.log(date)
   data = "INSERT INTO bookingstatus " +
      " VALUES ('" + 0 + "','" + req.session.mail + "','" + req.body.name + "','" + req.body.type + "','" + req.body.roomWant + "','" + 0 + "','" + date + "','" + checkOutDate + "','" + req.body.nama + "','" + req.body.nik + "','" + 0 + "')"
1
   data1 = "SELECT * " +
      " FROM  bookingstatus " +
      ...
      if (err) throw err;
         ...
         connectDB.query(data1, (err1, result) => {
            for (i in result) {
               ...
               var b = result[i].checkout
               b = b.toString()
               result[i].checkout = b.slice(0, 15);
            }
            res.render('user/statusShow', { user: req.session.mail, msg: "Kamar berhasil di booking, silahkan menunggu konfirmasi dari kami", err: "", data: result });
         })
      }
   })
}
...
exports.getShowStatus = (req, res, next) => {
   ...
   connectDB.query(data, (err, result) => {
      ...
      if (err) throw err;
         ...
         if (result.length < 1) {
            res.render('user/statusShow', { user: req.session.mail, msg: "", err: "Kamu tidak memiliki pesanan", data: result });
         }
         else {
...
exports.deleteBooking =(req,res,next)=>{
   ...
   " AND nama = "+mysql.escape(req.body.nama) +
   " AND nik = "+mysql.escape(req.body.nik)
